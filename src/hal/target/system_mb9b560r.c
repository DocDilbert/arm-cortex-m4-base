 ...
 ** 2014-08-15  0.6  EH   Added FPU settings & wait for Trace Buffer enable
 ******************************************************************************/
...
void SystemCoreClockUpdate(void)
{
    ...
    switch ((FM4_CRG->SCM_CTL >> 5U) & 0x07U)
    {
        ...
        case 2u: /* PLL clock                      */
            ...
            u32RegisterRead = (__CLKMO
                    * (((uint32_t) (FM4_CRG->PLL_CTL2) & 0x3Fu) + 1u));
            masterClk = (u32RegisterRead
                    / ((((uint32_t) (FM4_CRG->PLL_CTL1) >> 4ul) & 0x0Fu) + 1u));
            break;
...
// FPU settings 
    ...
    if (SCM_CTL_Val & (1UL << 3))
    { /* Sub clock oscillator enabled ? */
    ...
    if ((uint8_t) (SCM_CTL_Val & 0xE0u) == (uint8_t) 0x80ul)
    { /* Low-speed CR oscillation selected ? */
        ...
        if (u8IoRegisterRead > (uint8_t) 0ul)
        {
            ...
            for (i = 0; i < u32WaitValue; i++)
                ; /* wait for changing clock */
        }
